...
const RecruiterLogin: React.FC<RecruiterLoginProps> = ({ onSuccess, onSwitchToRegister }) => {
  const { login } = useAuth();
  ...
  try {
    ...
    if (response.data.user_role !== 'recruiter') {
      setError('Invalid credentials for recruiter login. Please use the Candidate tab.');
      setLoading(false);
      return;
    }
    ...
    // Clean up any stored token on error
